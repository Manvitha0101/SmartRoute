import { useState } from 'react';
import { useAuth } from '../context/AuthContext';
import * as api from '../api';
import { ApiError } from '../api/client';

export default function LoginPage() {
  const { login } = useAuth();
  const [email, setEmail] = useState('dispatcher@smartroute.io');
  const [password, setPassword] = useState('Password@123');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [googleLoading, setGoogleLoading] = useState(false);
  const [showGoogleModal, setShowGoogleModal] = useState(false);
  const [customGoogleEmail, setCustomGoogleEmail] = useState('');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setLoading(true);
    try {
      const data = await api.login(email, password);
      login(data.accessToken, data.user);
    } catch (err) {
      if (err instanceof ApiError) setError(err.message);
      else setError('Network error — is the backend server running?');
    } finally {
      setLoading(false);
    }
  };

  const handleGoogleSignIn = async (userEmail: string, userName?: string) => {
    setError('');
    setGoogleLoading(true);
    try {
      const data = await api.googleLogin({
        email: userEmail,
        name: userName || userEmail.split('@')[0],
        role: 'DISPATCHER',
      });
      login(data.accessToken, data.user);
    } catch (err) {
      if (err instanceof ApiError) setError(err.message);
      else setError('Google authentication failed. Please try again.');
    } finally {
      setGoogleLoading(false);
      setShowGoogleModal(false);
    }
  };

  const googleAccounts = [
    { email: 'dispatcher.south@smartroute.io', name: 'South Hub Dispatcher', avatar: '🚚' },
    { email: 'admin@smartroute.io', name: 'Master Logistics Admin', avatar: '⚡' },
    { email: 'suresh.reddy.driver@gmail.com', name: 'Suresh Reddy (Driver Portal)', avatar: '🛣️' },
  ];

  return (
    <div className="login-page">
      <div className="login-card">
        <div className="login-brand">
          <div className="login-logo-badge">🚀</div>
          <h1>SmartRoute</h1>
          <p>Intelligent Delivery Route Optimization & Fleet Dispatch</p>
        </div>

        {error && <div className="alert alert-error">{error}</div>}

        {/* ── Google Sign In Button ── */}
        <button
          type="button"
          className="google-btn"
          onClick={() => setShowGoogleModal(true)}
          disabled={googleLoading || loading}
        >
          <svg className="google-icon" viewBox="0 0 24 24" width="20" height="20">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
            />
          </svg>
          <span>{googleLoading ? 'Connecting to Google…' : 'Continue with Google'}</span>
        </button>

        <div className="login-divider">
          <span>or sign in with email</span>
        </div>

        <form onSubmit={handleSubmit} className="login-form">
          <div className="form-group">
            <label className="form-label" htmlFor="email">Email</label>
            <input
              id="email"
              type="email"
              className="form-input"
              value={email}
              onChange={e => setEmail(e.target.value)}
              required
              autoFocus
            />
          </div>

          <div className="form-group">
            <label className="form-label" htmlFor="password">Password</label>
            <input
              id="password"
              type="password"
              className="form-input"
              value={password}
              onChange={e => setPassword(e.target.value)}
              required
            />
          </div>

          <button type="submit" className="btn btn-primary" disabled={loading || googleLoading} style={{ marginTop: 8 }}>
            {loading ? <><span className="spinner" /> Signing in…</> : 'Sign In'}
          </button>
        </form>

        <div className="login-presets">
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Quick Demo Profiles:</span>
          <div style={{ display: 'flex', gap: '6px', marginTop: '4px' }}>
            <button
              type="button"
              className="btn btn-outline btn-xs"
              onClick={() => { setEmail('dispatcher@smartroute.io'); setPassword('Password@123'); }}
            >
              Dispatcher
            </button>
            <button
              type="button"
              className="btn btn-outline btn-xs"
              onClick={() => { setEmail('admin@smartroute.io'); setPassword('Password@123'); }}
            >
              Admin
            </button>
          </div>
        </div>
      </div>

      {/* ── Google Account Selector Modal ── */}
      {showGoogleModal && (
        <div className="modal-backdrop" onClick={() => setShowGoogleModal(false)}>
          <div className="modal-card google-modal" onClick={e => e.stopPropagation()}>
            <div className="google-modal-header">
              <svg viewBox="0 0 24 24" width="28" height="28">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
              </svg>
              <div>
                <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#1f2937' }}>Choose a Google Account</h3>
                <p style={{ margin: '2px 0 0', fontSize: '0.8rem', color: '#6b7280' }}>to continue to SmartRoute Logistics</p>
              </div>
            </div>

            <div className="google-accounts-list">
              {googleAccounts.map(acc => (
                <button
                  key={acc.email}
                  type="button"
                  className="google-account-row"
                  onClick={() => handleGoogleSignIn(acc.email, acc.name)}
                >
                  <span className="google-account-avatar">{acc.avatar}</span>
                  <div className="google-account-text">
                    <span className="google-account-name">{acc.name}</span>
                    <span className="google-account-email">{acc.email}</span>
                  </div>
                </button>
              ))}
            </div>

            <div style={{ marginTop: '12px', borderTop: '1px solid #e5e7eb', paddingTop: '12px' }}>
              <label style={{ fontSize: '0.8rem', color: '#4b5563', display: 'block', marginBottom: '6px' }}>
                Or use another Google email:
              </label>
              <div style={{ display: 'flex', gap: '8px' }}>
                <input
                  type="email"
                  className="form-input"
                  style={{ background: '#fff', color: '#111827', borderColor: '#d1d5db' }}
                  placeholder="name@gmail.com"
                  value={customGoogleEmail}
                  onChange={e => setCustomGoogleEmail(e.target.value)}
                />
                <button
                  type="button"
                  className="btn btn-primary btn-sm"
                  disabled={!customGoogleEmail.includes('@')}
                  onClick={() => handleGoogleSignIn(customGoogleEmail)}
                >
                  Continue
                </button>
              </div>
            </div>

            <button
              type="button"
              className="btn btn-outline btn-sm"
              style={{ marginTop: '16px', width: '100%', borderColor: '#d1d5db', color: '#4b5563' }}
              onClick={() => setShowGoogleModal(false)}
            >
              Cancel
            </button>
          </div>
        </div>
      )}
    </div>
  );
}
